...
import baseline.dataset_loaders.hainangibbonData as hainangibbonData
import baseline.dataset_loaders.teacherGibbonsound as teacherGibbonsound
...
def create_loader_for_simple_dataset(dataset_name=None,
    ...
    if dataset_name == 'libri1to3mix':
        ...
        data_loader = chime.Dataset(
            ...
            normalize_audio=False, n_samples=n_samples)
    elif dataset_name == 'hainangibbon':
        this_dataset_split = split
        data_loader = hainangibbonData.Dataset(
            #root_dir=hparams['gibbon_root'],  # 在 hparams.yaml 或 cmd 参数里定义 data_gibbon 路径
            sample_rate=hparams['fs'],
            timelength=hparams['audio_timelength'],
            split=this_dataset_split,
            augment=('train' in split),
            normalize_audio=False,
            n_samples=n_samples
        )
    elif dataset_name == 'teacherGibbonsound':
        this_dataset_split = split
        data_loader = teacherGibbonsound.Dataset(
            sample_rate=hparams['fs'],fixed_n_sources=fixed_n_sources,
            timelength=hparams['audio_timelength'],zero_pad=True,
            augment='train' in split,
            get_only_active_speakers=get_only_active_speakers,
        ...
        for dataset_name in hparams[data_split]:
            if data_split == "train":
                ...
                if dataset_name == 'chime':
                    ...
                    this_dataset_name = f"{data_split}_{dataset_name}"
                    generators[this_dataset_name] = data_loader.get_generator(
                        batch_size=hparams['batch_size'], num_workers=hparams['n_jobs']
                    )
                else:
