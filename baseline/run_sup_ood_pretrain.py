...
param_groups = get_parameter_groups(model, hparams['learning_rate'])
...
for i in range(hparams['n_epochs']):
    ...
    for cnt, (speakers, noise) in enumerate(train_tqdm_gen):
        ...
        crossCorrelation_l = correlatelossFunction(teacher_est_active_speakers, teacher_est_noises)
        l = 0.6 * speaker_l + 0.4 * noise_l + 0.2 * crossCorrelation_l
        l.backward()
