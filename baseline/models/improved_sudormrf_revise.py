...
# add a Lightweight call enhancement (LCE) module

# ------begin------
class LCEmodule(nn.Module):
    """
    Lightweight Denoising Front-end Module for Teacher Model
    """
    def __init__(self, in_channels=1, base_channels=8, num_layers=1, kernel_size=9):
        super(LCEmodule, self).__init__()

        layers = []
        # Step 1: initial conv
        layers.append(nn.Conv1d(in_channels, base_channels, kernel_size,
                                stride=1, padding=kernel_size // 2, bias=False))
        layers.append(nn.GroupNorm(1, base_channels))
        layers.append(nn.PReLU())

        # Step 2: depthwise-separable conv blocks
        for _ in range(num_layers):
            layers.append(DepthwiseSeparableConv(base_channels, base_channels, kernel_size))
            layers.append(nn.PReLU())
            layers.append(ECAAttention(base_channels))  # lightweight attention

        # Step 3: output projection (residual style)
        self.main = nn.Sequential(*layers)
        self.out_proj = nn.Conv1d(base_channels, 1, 1)  # map back to waveform dimension

    def forward(self, x):
        residual = x
        x = self.main(x)
        x = self.out_proj(x)
        return x + residual  # residual connection (denoising residual learning)
...
class ECAAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv(y.transpose(1, 2))    # temporal conv over channels
...
class SuDORMRF(nn.Module):
    def __init__(self,
        ...
        self.preLcemodule = LCEmodule(in_channels=1,base_channels=4, num_layers=1)
        # Front end
